...
# TRAIN TEST SPLIT
X = pd.read_csv(Path("data") / 'dat.csv', index_col = 0) # convert first column to rownames automatically
y = pd.read_csv(Path("data") / 'samples.csv', index_col = 0)
...
joblib.dump((X_train, X_test, y_train, y_test), Path("data") / "train_test_split.joblib")
